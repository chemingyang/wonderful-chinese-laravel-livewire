...
<div x-data="{ indx: $wire.index, maxindx: @js($maxindex) }" id="main-content" class="overflow-y-auto overflow-x-hidden top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 max-w-3xl max-h-full">
    <div x-show="indx == -1">
    ...
    <div x-data="{ idx: @js($idx) }" x-show="indx === idx">
        ...
        <div id="qa-{{$idx}}" class="space-y-6 p-3 section">
            @include('partials.homework.homework-start-module-lw',['type' => $lessonmodule->type, 'question' => $lessonmodule->question, 'idx' => $idx, 'rel' => $lessonmodule->id])
